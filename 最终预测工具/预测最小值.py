...
def transform_back(ts, fittedvalues, fc, conf, title):
    ...
    future_ARIMA.to_csv("气温预测/%d_min.csv" % (i + 1), index=True)

    import time
    work = pd.read_csv('气温预测/%d_min.csv' % (i + 1), encoding='utf-8')
    work.columns = ['date', 'tmin']
    ...
    if i==0:
        work.to_csv('气温预测/%d_min.csv' % (i + 1), index=None)
        work.to_csv("min/澳门_min.csv", mode='a+', index=None)
    else:
        work.to_csv('气温预测/%d_min.csv' % (i + 1), header=None, index=None)
        work.to_csv("min/澳门_min.csv", mode='a+', header=None, index=None)
...
if __name__ == '__main__':
    ...
    last = pd.read_csv('min/澳门_min.csv', encoding='utf-8')
    # last.sort_index(axis=0, ascending=True, by='date')
    last.sort_values('date', inplace=True)
    print(last['date'])
    last.to_csv("min/澳门_min.csv", index=None)